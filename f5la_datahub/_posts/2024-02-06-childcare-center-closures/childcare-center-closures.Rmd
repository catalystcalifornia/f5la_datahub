---
title: "Childcare Center Closures"
description: |
  Rapid response project during the COVID-19 pandemic to understand child care center capacity for child care centers and family child care centers in California and LA County between 2019 and 2020
author:
  - name: Catalyst California
    url: catalystcalifornia.orgyeah
date: 2024-02-06
categories:
  - Analysis
output:
...
```{r, include=FALSE}
##########CREATE A THEME FOR HIGHCHART GRAPHS###################
thm <- hc_theme(
  colors = c("#332a86", "#22bcb9", "#ec098d"),
  chart = list(
...
```{r, include=FALSE}
############################SET UP FOR CHILD CARE CENTER DATA##############################################

#read in csv data for child care centers

#childcare_org<-read.csv("W:/Project/RDA Team/First5LA/V2.0 Report/Data/ChildCareCenters11152020.csv") #download 11-15-20

childcare_org<-read.csv("W:/Project/RDA Team/First5LA/V2.0 Report/Data/ChildCareCenters01032021.csv")  #download 01/07/21
...
```{r include=FALSE}
#################################SET UP FOR FAMILY CHILD CARE HOME DATA#####################################

#########adding in Family Child Care Homes
   #read in family child care homes data
   homes<-read.csv("W:/Project/RDA Team/First5LA/V2.0 Report/Data/CHILDCAREHOMEmorethan801032021.csv")
...
```{r include=FALSE}
#now lets read both the child care and family childcare home data frames into excel for geocoding 
  library("writexl")
  write_xlsx(y2020, "W:/Project/RDA Team/First5LA/V2.0 Report/Data/childcarecenter_y2020_download010721_HCIMPORT.xlsx")
  write_xlsx(homes2020, "W:/Project/RDA Team/First5LA/V2.0 Report/Data/familyhomes_y2020_download010721_HCIMPORT.xlsx")
```
...
For this research, I pulled child care center and family child care home data from the [California Department of Social Services (CDSS) page](https://www.ccld.dss.ca.gov/carefacilitysearch/DownloadData). The data was last downloaded on 01/07/21 and is updated on the CDSS page weekly. 
...
__While this research focuses on permanently closed facilities, the facilities that are open are being impacted by Covid-19.__ [Anecdotes from providers and families](https://www.advancementprojectca.org/blog) reveal that many facilities are heavily impacted by Covid-19, which the data might not accurately capture. [Women of color and immigrant women](https://nwlc-ciw49tixgw5lbab.stackpathdns.com/wp-content/uploads/2020/08/ChildCareWorkersFS.pdf) in particular, who disproportionately represent child care workers, are shouldering the burden for providing early learning and care to families. 

For more information on how Covid-19 has been impacting child care in California and Los Angeles, please refer to the Center for the Study of Child Care and Employment's (CSCCE) [survey of licensed child care centers and licensed family child care programs](https://cscce.berkeley.edu/regional-snapshot-california-child-care-at-the-brink-covid-19/).
...
centers<-read_xlsx("W:/Project/RDA Team/First5LA/V2.0 Report/Data/childcarecenter_y2020_download010721_HCEXPORT.xlsx")
...
family<-read_xlsx("W:/Project/RDA Team/First5LA/V2.0 Report/Data/familyhomes_y2020_download010721_HCEXPORT.xlsx")
...
addCircleMarkers(data = centers, lat = ~Y, lng = ~X, 
                   ...
                   color="white", popup = ~htmlEscape(points), fillColor= "#332a86", stroke = TRUE,
  ...
  addCircleMarkers(data = family, lat = ~Y, lng = ~X, 
                   ...
                   color="white", popup = ~htmlEscape(points), fillColor= "#22bcb9", stroke = TRUE,
  ...
  #legend
   addLegend("bottomleft", colors = c("#22bcb9", "#332a86"), 
...
#Closures in LA
   ...
   #get the total closures lost in 2020 for CA and LAC
...
#childcare closures in CA in 2019
...
#childcare closures in LA county in 2019
   ...
   #now summarise total closures in 2019 for CA and LAC
  ...
  #combine 2019 and 2020 childcare center closures for CA and LAC
  
  closed_childcarecenter<-rbind(closed2019, closed2020) %>%
      mutate(closed=scales::comma(closed))%>%
  mutate(type="Child Care Center-Closures")
  
  #convert to wide
closed_childcarecenter<-spread(closed_childcarecenter, geography, closed)
  
 ###################################CHILD CARE CENTERS: SEATS LOST#######################################################
...
seats_childcarecenter<-rbind(seats2019, seats2020)%>%
  mutate(seatslost=scales::comma(seatslost))%>%
  mutate(type="Child Care Center-Seats Lost")

#convert to wide
seats_childcarecenter<-spread(seats_childcarecenter, geography, seatslost)

  
 ##################################FAMILY CHILD CARE HOMES: CLOSURES######################################################
  
  
#Closures in CA

  homes_closed2020<-homes2020%>%
    filter(open==0)%>% #only facilities that are closed
    group_by(month)%>%
    summarise(closed=sum(close))%>%
    mutate(geography='California')
...
#Closures in LA
   ...
   homes_closed2020_lac<-homes2020%>%
    filter(open==0)%>%
    filter(County.Name=='LOS ANGELES')%>%
    group_by(month)%>%
    summarise(closed=sum(close))%>%
   ...
   homes_closed2020<-rbind(homes_closed2020, homes_closed2020_lac)
   
   #get the total closures lost in 2020 for CA and LAC
   
   homes_closed2020<-homes_closed2020%>%
     mutate(year='2020')%>%
     select(year, geography, closed)%>%
     group_by(year, geography)%>%
     summarise(closed=sum(closed))
...
#childcare closures in LA county in 2019
    ...
    homes_closed2019_lac<-homes2019%>%
    filter(open==0)%>%
    filter(County.Name=='LOS ANGELES')%>%
    group_by(month)%>%
    summarise(closed=sum(close))%>%
   ...
   homes_closed2019<-rbind(homes_closed2019, homes_closed2019_lac)
   
   #now summarise total closures in 2019 for CA and LAC
   
  homes_closed2019<-homes_closed2019%>%
     mutate(year='2019')%>%
     select(year, geography, closed)%>%
     group_by(year, geography)%>%
     summarise(closed=sum(closed))
  ...
  closed_familyhome<-rbind(homes_closed2020, homes_closed2019)%>%
          mutate(closed=scales::comma(closed))%>%
  mutate(type="Family Child Care Home-Closures")
  
  #convert to wide
  
closed_familyhome<-spread(closed_familyhome, geography, closed)

 
##################################FAMILY CHILD CARE HOMES: SEATS LOST###################################################
 
#CA seats 
  
  home_seats2020<-homes2020%>%
    filter(open==0)%>% #only facilities that are closed
    group_by(month)%>%
    summarise(seatslost=sum(Facility.Capacity))%>%
    mutate(geography='California')
  ...
  #LA County seats
   ...
   home_seats2020<-rbind(home_seats2020, home_seats2020_lac)
   
   #get the total seats lost in 2020 for CA and LAC
   
   home_seats2020<-home_seats2020%>%
     mutate(year='2020')%>%
     select(year, geography, seatslost)%>%
     group_by(year, geography)%>%
     summarise(seatslost=sum(seatslost))
...
##now repeat everything above for 2019#
  ...
  home_seats2019<-homes2019%>%
   ...
   home_seats2019<-rbind(home_seats2019, home_seats2019_lac)
   
   #now summarise total seats lost in 2019 for CA and LAC
   
  home_seats2019<-home_seats2019%>%
     mutate(year='2019')%>%
     select(year, geography, seatslost)%>%
     group_by(year, geography)%>%
     summarise(seatslost=sum(seatslost))
...
seats_familyhome<-rbind(home_seats2019, home_seats2020)%>%
  mutate(seatslost=scales::comma(seatslost))%>%
  mutate(type="Family Child Care Home-Seats Lost")

#convert to wide
seats_familyhome<-spread(seats_familyhome, geography, seatslost)
...
table<-rbind(closed_familyhome, closed_childcarecenter, seats_familyhome, seats_childcarecenter)


#customize order of the rows to be alphabetical
table <- table[order(table$type),]
...
#make a nice table
kable(table, 
      caption = "Summary of Permanent Closures and Seats Lost", align='c', col.names = c("", "", "California", "Los Angeles County"))%>%
      kable_styling(bootstrap_options = c("striped"), position = "center", font_size = 15)%>%
    pack_rows("Permanent Center Closures", 1, 4, label_row_css = "background-color: #332a86; color: #fff;") %>%
      pack_rows("Family Child Care Home", 1, 2)%>%
  pack_rows("Child Care Center", 3, 4)%>%
      pack_rows("Seats Lost", 5, 8, label_row_css = "background-color: #332a86; color: #fff;")%>%
      pack_rows("Family Child Care Home", 5, 6)%>%
  pack_rows("Child Care Center", 7, 8)
...
closed2019_lac<-childcare%>%
  ...
  geom_line(aes(group=1))+
     geom_point()+
     geom_hline(aes(yintercept=mean), colour="#22bcb9", lty=5)+
    
      geom_text(data=text, color="black",
...
#graph
   ...
   hc<-hchart(homesclosed2020, "line",
          hcaes(x=MonthName, y=closed, group=geography),
