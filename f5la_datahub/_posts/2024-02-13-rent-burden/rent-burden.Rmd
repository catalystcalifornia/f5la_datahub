...
Note: Aggregated data above conceal variations with disaggregated subgroups. See chart below for details.
...
hc_asian <- d_chart_asian %>%
  ...
  hc_yAxis(max = 100) %>%
  hc_title(
    text = "Afghan children under 5 are more likely to be rent burdened"
  ) %>%
   hc_exporting(
    enabled = TRUE, sourceWidth=900, sourceHeight=600,
    chartOptions=list(plotOptions=list(series=list(dataLabels=list(enabled=TRUE, format='{point.y:,.1f}%')))),
